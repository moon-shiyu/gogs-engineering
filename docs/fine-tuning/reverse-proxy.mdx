...
<Tip>
  Gogs strips the subpath of `EXTERNAL_URL` from incoming requests on its own, so routes, static assets, and the web frontend resolve identically whether or not the reverse proxy strips the prefix. The examples below strip the prefix at the proxy, but proxying the full path through also works.
</Tip>
